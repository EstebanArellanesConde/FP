...
/**
 * @brief reserva memoria para la parte dinámica de almacenamiento
 * 
 ...
 *  
 * @param n numero de elementos a incrementar o reducir 
...
float **resize(int n){
    ...
    float **memory;
    memory=(float**)malloc(complex_dynamic_memory, 2*tam*sizeof(float));
    return memory;
}
...
void open_in_file(char *filename){
    ...
    if(!in_file){
        printf("No tiene permisos suficientes para abrir %s\n", filename);
    }
}
...
/*
    ...
    si está en la memoria dinámica, redimensiona el tamaño eliminando 
    el útlimo elemento.
*/
void memory_delete(){ 
    if (countMemory < 10){
...
void memory_read(float c[2], int n){ 
    if (n < countMemory){
        if (n < 10){
            ...
            c[0] = complex_static_memory[n-10][0];
            c[1] = complex_static_memory[n-10][1];
        }
    }else{
